...
var AudioRecorder = (function (_Component) {
  ...
  _createClass(AudioRecorder, [{
    ...
    key: 'moveSlider',
    value: function moveSlider(time) {
      console.log(time);
      var trimStart = time[0];
      var trimEnd = time[1];
      this.setState({
        trimStart: trimStart,
        trimEnd: trimEnd
    ...
    value: function render() {
      ...
      if (this.props.download) {
        audioButtons.push(_react2['default'].createElement(
          'button',
          { type: 'button', key: 'download', className: downloadButtonClass.join(' '), onClick: this.downloadAudio.bind(this) },
          strings.download
        ));
      }
      ...
      if (this.props.onSave) {
        audioButtons.push(_react2['default'].createElement(
          'button',
          { type: 'button', key: 'save', className: downloadButtonClass.join(' '), onClick: this.saveAudio.bind(this) },
          strings.save
        ));
      }
      ...
      return _react2['default'].createElement(
        ...
        _react2['default'].createElement(
          'button',
          { type: 'button', key: 'remove', className: removeButtonClass.join(' '), onClick: this.removeAudio.bind(this) },
          strings.remove
        ),
        audioButtons,
        _react2['default'].createElement(
          _reactSlider2['default'],
          { min: 0, max: 1, onChange: this.moveSlider.bind(this), defaultValue: [0, 1], minDistance: 0.001, step: 0.001, withBars: true },
